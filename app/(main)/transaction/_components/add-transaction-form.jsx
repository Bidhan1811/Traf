 ...
 const handleScanComplete = (scannedData) => {
  if(scannedData) {
    ...
    if(scannedData.category) {
      // Find the category that matches the scanned category name
  ...
  return (
    <form className='space-y-6 pb-5' onSubmit={handleSubmit(onSubmit)}>
      {/*ai receipt scanner*/}
    ...
    <div className='gap-6 grid md:grid-cols-2'>
        ...
        >
          ...
          <SelectContent>
            {accounts.map((account)=>{
              return <SelectItem key={account.id} value={account.id}>{account.name} (INR {parseFloat(account.balance).toFixed(2)})</SelectItem>
            })}
            <CreateAccountDrawer>
      ...
      <div className='space-y-4 flex items-center justify-between rounded-lg border p-3'>
        ...
        <p className='text-sm text-muted-foreground'>Setp up a recurring schedule for this transaction</p>
        </div>
        <Switch checked={isRecurring}  onCheckedChange={(checked) => setValue("isRecurring",checked)}>Set As Default</Switch>
      </div>
      ...
      {isRecurring && (
        ...
        <div className='flex gap-4'>
          <Button 
            className="bg-gradient-to-br from-blue-500 to-black w-full" 
            type="submit" 
          ...
          <Button className="bg-red-700 w-full" type="button" onClick={() => router.back()}>Cancel</Button>
        </div>
