...
export const processRecurringTransaction = inngest.createFunction({
    ...
    async ({step, event}) => {
        if(!event?.data?.transactionId || !event?.data?.userId) {
            console.error("Missing transactionId or userId");
            ...
            await step.run(`generate-report-${user.id}`, async () => {
                ...
                if (stats.transactionCount > 0) {
                    ...
                    try {
                        ...
                        console.log(`Successfully sent monthly report to ${user.email}`);
                    } catch (emailError) {
                        console.error(`Failed to send email to ${user.email}:`, emailError);
                        throw emailError; // Re-throw to be caught by the outer try-catch
                    }
                } else {
                    console.log(`No transactions found for user ${user.id} in ${monthName}`);
                }
            });
        } catch (error) {
...
async function generateFinancialInsights(stats, month) {
  ...
  try {
    ...
    return [
        ...
        "Track your recurring expenses to identify areas for savings.",
    ];
  }
}
