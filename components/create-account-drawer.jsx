    ...
    <div className='px-4 pb-4'>
        <form className='space-y-4' onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className='space-y-4 flex items-center justify-between rounded-lg border p-3'>
            ...
            <label htmlFor='isDefault' className='text-sm font-medium cursor-pointer'>Set this Account as Default</label>
            <p className='text-sm text-muted-foreground'>This account will be selected by default for transactions</p>
            </div>
            <Switch id='isDefault' onCheckedChange={(checked) => setValue("isDefault", checked)} checked={watch('isDefault')}>Set As Default</Switch>
          </div>
          ...
          <div className='flex gap-4 pt-2'>
            ...
            <Button type="submit" className="flex-1" disabled={createAccountLoading}>{createAccountLoading? (<Loader2 className="mr-2 h-4 w-4 animate-spin">Creating...</Loader2>):("Create Account") }</Button>
          </div>
          </div>
        </form>
    </div>
